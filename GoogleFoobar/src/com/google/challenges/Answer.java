...
public class Answer {
	...
	public static void GenerateLookup(){
		xValues = new double[(2*MAX_SIZE) * (2*MAX_SIZE)];
		yValues = new double[(2*MAX_SIZE) * (2*MAX_SIZE)];
		// how to generate the vectors?
		HashSet<Double> slopes = new HashSet<>();
		HashMap<Double, HashSet<Double>> set = new HashMap<>();
		double slope = 0;
		for(int i = -1; i >= -MAX_SIZE; i--){
			for(int j = -1; j >= -MAX_SIZE; j--){
				slope = (double)j/(double)i;
				
				if(!slopes.contains(slope)){
					xValues[iterations] = i;
					yValues[iterations] = j;
					iterations++;
					slopes.add(slope);
	...
	public static int answer(int[] dimensions, int[] captain_position, int[] badguy_position, int distance) { 
		...
		*/
		...
		for(int x = -bigger; x <= -1; x++){
			for (int y = -bigger; y <= bigger; y++){
				// disregard straight lines, they just bounce back to the origin 
				if(y==0)
					continue;
				int iterations = 1;
				if(dataMap[Math.abs(x)][Math.abs(y)]==0 ){//|| dataMap[Math.abs(y)][Math.abs(x)]== 1){
				...
				*/
				...
				while(iterations < maxIterations && workingDistance + referenceLength <= distance){
					...
					if(endPoint[1] - margin <= resultY && resultY <= endPoint[1] + margin){
						int valX = (int)currentOrigin[0];
						int valY = (int)currentOrigin[1];
						if(originMap[valX][valY]==0){
							possiblePaths += ((workingDistance += referenceLength) <= distance) ? 1 : 0;
							originMap[valX][valY] = 1;
						}
						break;
					}
					referenceLength = Math.sqrt((cndx*cndx)+(cndy*cndy));
		...
		System.out.println("total skipped slopes: " + slopeSkippedCount);
